    ...
    <div>
      {items.map((item, i) => (
        <a key={i} href={item.link} target="_blank" rel="noopener noreferrer"
          style={{display:'block', padding:'10px 14px', borderBottom:'1px dashed var(--border)', textDecoration:'none', transition:'background 0.15s'}}
...
export default function Home() {
  ...
  function getFeaturedPreview(post) {
    ...
    return { first: (parts[0] || '') + '</p>', second: parts.length > 1 ? (parts[1] || '') + '</p>' : '' }
  }
                ...
                {webringLinks.length > 0 ? (
                  <>
                    <div className="webring">
                      <button onClick={() => setWebringIndex(i => (i - 1 + webringLinks.length) % webringLinks.length)}
                      ...
                      <button onClick={() => setWebringIndex(i => (i + 1) % webringLinks.length)}
                        style={{background:'none',border:'1px solid var(--border)',color:'var(--link-blue)',fontFamily:'VT323, monospace',fontSize:'15px',padding:'2px 6px',cursor:'pointer'}}>
                        next ▶
                      </button>
                    </div>
                    <div style={{textAlign:'center',marginTop:'8px'}}>
                      <a href={webringLinks[webringIndex]?.url} target="_blank" rel="noopener noreferrer" className="webring-join">[ visit this site ]</a>
                    </div>
          ...
          <main className="main-content">
            ...
            {featuredPost ? (
              ...
              <article className="hero-story">
                ...
                <div className="story-columns">
                  <p className="drop-cap">{settings.bio || 'A 40-something polymath and serial hobbyist.'}</p>
                  <p>This is my corner of the internet. No timeline. No ratio. No algorithm. Subscribe to hear from me directly.</p>
                </div>
          ...
          <aside className="sidebar-right">
            ...
            <div className="newsletter-widget">
              ...
              <div className="newsletter-body">
                ...
                <div className="newsletter-sub">New posts, occasional thoughts, and whatever I&apos;m obsessing over — delivered directly. No tracking pixels. No selling your data.</div>
